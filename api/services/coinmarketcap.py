...
from typing import Set, List, Dict, Optional, ClassVar
from pydantic import BaseModel
from schemas import Wallet, QuotedWallet, Balance, QuotedBalance
...
class CoinMarketCapAsset(_CoinMarketCapBaseItem):
    ...
    quote: Optional[Dict[str, CoinMarketCapQuote]] = None
...
class CoinMarketCapQuotes(BaseModel):
    ...
    data: Dict[str, CoinMarketCapAsset]  # Keys are asset ids.

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        # if https://github.com/samuelcolvin/pydantic/pull/2625 is merged, ComputedField can be used
        object.__setattr__(self, '_symbol_asset_map', {asset.symbol: str(asset.id) for asset in self.data.values()})
    ...
    def _get_price(self, symbol: str, target_currency: str = 'USD') -> float:
        try:
            ...
            return ValueError(f'No quote available for {symbol}')

    def _get_quoted_balance(self, balance: Balance, target_currency: str = 'USD') -> QuotedBalance:
        price = self._get_price(balance.symbol, target_currency)
        return QuotedBalance.from_balance(balance, price)

    def get_quoted_wallet(self, wallet: Wallet, target_currency: str = 'USD') -> QuotedWallet:
...
def get_quoted_wallet(wallet: Wallet) -> QuotedWallet:
    ...
    symbols = {balance.symbol for balance in wallet.balances if balance.symbol not in UNKNOWN_ASSET_SYMBOLS}
    coinmarketcap_map = _get_coinmarketcap_map(symbols)  # TODO might be done via a database lookup
