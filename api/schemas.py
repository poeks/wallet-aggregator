...
class Balance(BaseModel):
    symbol: str = Field(
        ..., description="Symbol refering to the asset, based on CoinMarketCap symbol."
    )
    amount: float = Field(..., description="Amount of the token balance.")


class _BaseWallet(BaseModel):
...
class QuotedBalance(Balance):
    ...
    value: float = Field(
        None, description="Value of the balance (amount * unit price)"
    )  # Defined so it gets included in the docs.
...
class QuotedWallet(_BaseWallet):
    ...
    def __init__(self, **kwargs):
        ...
        object.__setattr__(
            self, "total_value", sum(balance.value for balance in self.balances)
        )


class WalletsCurrent(BaseModel):
    data: List[QuotedWallet] = Field(
        ...,
        description="Collection of wallets (can be actual wallet addresses or exchange wallets).",
    )
